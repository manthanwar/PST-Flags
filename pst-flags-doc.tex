...
As prerequisites for \texttt{pst-flags} you need a working
...
Flags are in many different height to width aspect ratio (AR). The flags of India \verb|\flagIN| and UK \verb|\flagUK| uses the aspect ratios of 2:3 and 1:2 respectively. These are the two of the most common aspect ratios used by over 80 and 50 countries respectively. The accuracy of colors, shapes, internal objects and aspect ratio make the flag of a sovereign state unique. Width is the only option needed to draw the flag with macro appended with two letter country code which internally determines the corresponding height. In order to draw a flag with the specific height use the aspect ratio from the table below to calculate the appropriate width and supply it rectangular bracket in centimeters. \texttt{PSTricks} macro \verb|\rput| macro and environment \verb|pspictur| can be also used and encouraged for the accurate placement.
...
\paragraph{Colors:} Flag colors are defined using country code appended with color name which can be used elsewhere, e.g., ukRed, inGreen, usBlue, deYellow, etc. \verb|\pscircle[linecolor=usRed](0,0){2}|
